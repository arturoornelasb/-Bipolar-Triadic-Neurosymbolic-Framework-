...
class TriadicRelationalFramework:
    ...
    # Método generativo necesario para la inferencia
    def generative(self, C1: int, C2: int, C3: int, a: int, b: int) -> TriadicResult:
        # Formula: C4 = (a * C2 * C3) / (b * C1)
        num = a * C2 * C3
        den = b * C1
        if den == 0: raise ValueError("Division by zero in generative mode")
        if num % den != 0: raise ValueError("Result is not an integer")
        C4 = num // den
        K = Fraction(1, a * b)
